...
def inference_test_msd(model, X_test, y_test, save_dump = False, fold_number = 0):
    ...
    for i, y_true in zip(np.arange(len(y_test)), y_test):
        ...
        AGGREGATE_GT += y_true
        AGGREGATE_PRED += list(pred)
    # print("prediction == ",AGGREGATE_PRED," True == ",AGGREGATE_GT)
    accuracy = accuracy_score(AGGREGATE_GT, AGGREGATE_PRED)*100
    prec = precision(AGGREGATE_GT, AGGREGATE_PRED)*100
    rec = recall(AGGREGATE_GT, AGGREGATE_PRED)*100
    f1 = f1_score(prec, rec)

    print("Accuracy Mean (sd) {:.2f}  ".format(accuracy))
    print("Precision Mean (sd) {:.2f} ".format(prec))
    print("Recall Mean (sd) {:.2f} ".format(rec))
    print("F1 Mean (sd) {:.2f} ".format(f1))
